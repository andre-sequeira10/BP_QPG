...
parser.add_argument('--inpt_scaling', type=int, default=0)     
parser.add_argument('--local_measurements', type=int, default=0)     
...
save = args.save
inpt_scaling = args.inpt_scaling
local_measurements = args.local_measurements
...
def ansatz(state, weights, input_scaling=0 ,n_layers=1, change_of_basis=False, entanglement="all2all"):
...
class policy_estimator_q(nn.Module):        
    def __init__(self, env, inpt_scaling = 0 , local_measurements=False):
        super(policy_estimator_q, self).__init__()
        self.local_measurements = local_measurements
        self.inpt_scaling = inpt_scaling
        #@qml.batch_input(argnum=0)
        if self.local_measurements == 0:
            if self.inpt_scaling == 0:
                @qml.qnode(device, diff_method="backprop")
                def qcircuit_training(inputs, weights0):
                    ...
                    return qml.expval(qml.PauliZ(wires=3))# @ qml.PauliZ(wires=1) @ qml.PauliZ(wires=2) @ qml.PauliZ(wires=3))
            else:
                @qml.qnode(device, diff_method="backprop")
                def qcircuit_training(inputs, weights0, inpt_scaling):
                    
                    for q in range(n_qubits):
                        qml.Hadamard(wires=q)
                            
                    ansatz(inputs, weights0,input_scaling=inpt_scaling, n_layers=n_layers, entanglement=ent)
                    for i in range(n_qubits-1):
                        qml.CNOT(wires=[i,3])
        ...
        if policy == "Q":
            if self.inpt_scaling == 1:
                weight_shapes = {"weights0":(n_layers, n_qubits,2),"inpt_scaling":(n_layers,2,n_qubits)}#,"weights2":(n_layers,n_qubits,3),"weights3":(n_layers,n_qubits,3),"weights4":(n_layers,n_qubits,3)}#, "weights5":(1,n_qubits,3)}
            else:
                weight_shapes = {"weights0":(n_layers, n_qubits,2)}
    ...
    def forward(self, state):
        ...
        if policy == "Q":
            ...
            if self.local_measurements==0:
                outs = torch.stack([out,-out])
                action_probs = F.softmax(3*outs, dim=-1)
            else:   
                s0 = torch.sum(out) 
                #s1 = torch.mean(out[2:]) 
                outs = torch.stack([s0,-s0])
            
                action_probs = F.softmax(outs, dim=-1)
...
pe_q= policy_estimator_q(env,inpt_scaling=inpt_scaling, local_measurements=local_measurements)
...
np.save("cartpole_n3_{}_inpt_scaling-{}_n_layers-{}||{}.npy".format(local_measurements,inpt_scaling,n_layers,str(processid)), rewards_q)
np.save("cartpole_n3_{}_inpt_scaling-{}_n_layers-{}_grads_norm||{}.npy".format(local_measurements,inpt_scaling,n_layers,str(processid)), grads_q)
np.save("cartpole_n3_{}_inpt_scaling-{}_n_layers-{}_grads_var||{}.npy".format(local_measurements,inpt_scaling,n_layers,str(processid)), vars)
